...
var KindleCompression = (function () {
  ...
  function j(c, e) {
    ...
    for (var r = 0; r < e.length; ) {
      ...
      r++;
      o.charCodeAt(0) <= d
        ? c.hasOwnProperty(l + o)
          ? (l += o)
          : (l.length > 0 && h < f && ((c[l + o] = h), h++, h === a && (h = g)),
            (l = o))
        : (l = "");
    }
    return c;
  ...
  return {
    lzCompress: function (k) {
      ...
      for (var t = 0; t < k.length; ) {
        ...
        if (u.charCodeAt(0) <= d) {
          for (; o.length > 0; ) {
            ...
            o = o.substr(q);
            l.push(c + q);
            for (q = 0; q < s.length; q++) l.push(s.charCodeAt(q));
          }
          e.hasOwnProperty(h + u)
            ? (h += u)
            : (h.length > 0 &&
                (l.push(h.length === 1 ? h.charCodeAt(0) : e[h]),
                r < f && ((e[h + u] = r), r++, r === a && (r = g))),
              (h = u));
        } else
          h.length > 0 &&
            (l.push(h.length === 1 ? h.charCodeAt(0) : e[h]), (h = "")),
            (o += u);
      }
      for (
        h.length > 0 && l.push(h.length === 1 ? h.charCodeAt(0) : e[h]);
        o.length > 0;
      ...
      for (i = 0; i < l.length; i++) l[i] = String.fromCharCode(l[i]);
      return l.join("");
    },
    lzExpand: function (k) {
      for (var e = {}, l = [], h, r = b, o = "", q, s = 0; s < k.length; ) {
        h = k.charCodeAt(s);
        s++;
        if (h <= d) h = String.fromCharCode(h);
        else if (h >= b) (h = e[h]) || (h = o + q);
        else {
          o = h - c;
          l.push(k.substr(s, o));
          s += o;
        ...
        l.push(h);
        q = h.charAt(0);
        r < f && o.length > 0 && ((e[r] = o + q), r++, r === a && (r = g));
        o = h;
      }
      return l.join("");
    },
    ...
    lzAddNumbersToDictionary: function (a, b) {
      for (var c = [], g = 100; g < 1e3; g++) c.push("" + g);
      return h(c, a, b);
    },
    lzCompressWithStaticDictionary: function (a, b) {
      ...
      for (var s = 0; s < a.length; ) {
        ...
        if (t.charCodeAt(0) <= d) {
          for (; h.length > 0; ) {
            ...
            h = h.substr(o);
            g.push(c + o);
            for (o = 0; o < q.length; o++) g.push(q.charCodeAt(o));
          }
          b.hasOwnProperty(f + t)
            ? (f += t)
            : (f.length > 0 && g.push(f.length === 1 ? f.charCodeAt(0) : b[f]),
              (f = t));
        } else
          f.length > 0 &&
            (g.push(f.length === 1 ? f.charCodeAt(0) : b[f]), (f = "")),
            (h += t);
      }
      for (
        f.length > 0 && g.push(f.length === 1 ? f.charCodeAt(0) : b[f]);
      ...
      for (i = 0; i < g.length; i++) g[i] = String.fromCharCode(g[i]);
      return g.join("");
    },
...
function usageExit(errCode = 0) {
  ...
  console.log(
    "Converts kindle books in a given Chrome/Chromium WebSQL file to HTML file(s)."
  );
  console.log(" -f: Name of an sqlite file created by kindle cloud reader.");
  console.log(
    "     Example for a typical path: ~/.config/google-chrome/Default/databases/https_read.amazon.com_0/2"
  );
  console.log(
    "     If you are using Chromium, look at ~/.config/chromium/Default/databases/ instead"
  );
...
db.all("select metadata from 'bookinfo'", function (err, rows) {
  ...
  rows.forEach(function (row) {
    ...
    const authors = metadata.authorList.join(",");
    // used for dictionary request at https://read.amazon.com/dict/getDefinition?asin=<asin>&word=<word>
    const asin = metadata.asin;
    const ca = s(metadata);

    const contentType = `<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">`;
    const authorstr = `<meta name="author" content="${authors}">`;
    const titlestr = `<title>${title}</title>`;
    const hypothesis = `<script type="text/javascript" src="https://hypothes.is/embed.js" async ></script>`;
    const bodyId = `<body id="${asin}">`;
    // const HtmlHeader = `<html><head>${css}${contentType}${authorstr}${titlestr}${hypothesis}</head><body id="${asin}>`;
    ...
    function appendUncompressedData() {
      return new Promise(function (resolve, reject) {
        db.all(
          "select id, piece, other from 'fragments' where asin='" +
            asin +
            "' order by id",
          function (err, rows) {
            const v = rows.forEach(function (row) {
              var id = row.id;
    ...
    appendUncompressedData().then(function resp(res) {
